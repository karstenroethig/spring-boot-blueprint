...
import karstenroethig.springbootblueprint.webapp.bean.search.UserSearchBean;
...
public class UserAdminController extends AbstractController
{
	...
	public String list(Model model, @PageableDefault(size = 20, sort = User_.EMAIL) Pageable pageable)
	{
		Page<OldUserDto> resultsPage = userService.findBySearchParams(userSearchBean.getUserSearchDto(), pageable);
	...
	{
		OldUserDto user = userService.find(id);
	...
	public String delete(@PathVariable("id") Long id, final RedirectAttributes redirectAttributes, Model model)
	{
		...
		if (userService.delete(id))
			redirectAttributes.addFlashAttribute(AttributeNames.MESSAGES,
					Messages.createWithSuccess(MessageKeyEnum.USER_ADMIN_DELETE_SUCCESS, user.getEmail()));
		else
			redirectAttributes.addFlashAttribute(AttributeNames.MESSAGES,
				Messages.createWithError(MessageKeyEnum.USER_ADMIN_DELETE_ERROR, user.getEmail()));

		return UrlMappings.redirect(UrlMappings.CONTROLLER_USER_ADMIN, UrlMappings.ACTION_LIST);
	}

	@PostMapping(value = UrlMappings.ACTION_SAVE)
	public String save(@ModelAttribute(AttributeNames.USER) @Valid OldUserDto user, BindingResult bindingResult,
		final RedirectAttributes redirectAttributes, Model model)
	{
		...
		{
			redirectAttributes.addFlashAttribute(AttributeNames.MESSAGES,
					Messages.createWithSuccess(MessageKeyEnum.USER_ADMIN_SAVE_SUCCESS, user.getEmail()));
			return UrlMappings.redirect(UrlMappings.CONTROLLER_USER_ADMIN, UrlMappings.ACTION_LIST);
		}
		...
		final RedirectAttributes redirectAttributes, Model model)
	{
		...
		{
			redirectAttributes.addFlashAttribute(AttributeNames.MESSAGES,
					Messages.createWithSuccess(MessageKeyEnum.USER_ADMIN_UPDATE_SUCCESS, user.getEmail()));
			return UrlMappings.redirect(UrlMappings.CONTROLLER_USER_ADMIN, UrlMappings.ACTION_LIST);
		}
